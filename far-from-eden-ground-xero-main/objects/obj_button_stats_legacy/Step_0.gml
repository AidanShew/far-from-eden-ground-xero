...
if (hide_stats==true&&transition==true) {
	if (!instance_exists(obj_button_horde)) {
		instance_destroy(obj_career);
		instance_create_layer(928, 576, "Instances", obj_button_horde);
		obj_button_horde.image_alpha=0;
		instance_create_layer(928, 672, "Instances", obj_button_controls);
		obj_button_controls.image_alpha=0;
	...
	if (obj_button_horde.image_alpha<1) {
		...
		obj_button_controls.image_alpha+=0.25;
		obj_button_quit.image_alpha+=0.25;
		draw_set_alpha(text_alpha);
		if (text_alpha<=0) {text_alpha+=0.5}
		if (obj_button_career.y>768) { obj_button_career.y-=50; }
	}
	else {
		obj_button_horde.image_alpha=1;
		obj_button_controls.image_alpha=1;
...
if (left_press==true) {
	if (instance_exists(obj_button_horde)) {
		if (obj_button_horde.image_alpha<=0) {
			...
			obj_button_controls.image_alpha-=0.25;
			obj_button_quit.image_alpha-=0.25;
			if (obj_button_career.y<864) { obj_button_career.y+=50; }
			draw_set_alpha(text_alpha);
